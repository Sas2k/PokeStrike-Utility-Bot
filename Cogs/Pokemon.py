...
class Pokemon(commands.Cog):
    ...
    @commands.command(name="ptcg", brief="pokemon tcg card finder", description="Finds the card that matches the id \nex** xy1-1")
    async def ptcg(self, ctx: commands.Context, id):
        ...
        embed.add_field(name="Moves [for now only 1 is dsiplayed]", value=f"{card.attacks[0].name} | {card.attacks[0].text} | {card.attacks[0].cost} | {card.attacks[0].damage}", inline=False)
